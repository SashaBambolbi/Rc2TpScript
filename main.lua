...
local function load()
    local j=player:GetAttribute("PData")
    if j and j~=""then
        local d=H:JSONDecode(j)
        points=d.p or {}
        binds=d.b or {}
    else
        points={}
...
local function save()
    player:SetAttribute("PData",H:JSONEncode({p=points,b=binds}))
end

local function addPoint(n)
    local p=root.Position
    points[n]={X=math.floor(p.X*10)/10,Y=math.floor(p.Y*10)/10,Z=math.floor(p.Z*10)/10}
    save()
    update()
end
...
local keys={[Enum.KeyCode.One]="1",[Enum.KeyCode.Two]="2",[Enum.KeyCode.Three]="3",[Enum.KeyCode.Four]="4",[Enum.KeyCode.Five]="5",[Enum.KeyCode.Six]="6",[Enum.KeyCode.Seven]="7",[Enum.KeyCode.Eight]="8",[Enum.KeyCode.Nine]="9",[Enum.KeyCode.Zero]="0",[Enum.KeyCode.Q]="Q",[Enum.KeyCode.E]="E",[Enum.KeyCode.R]="R",[Enum.KeyCode.T]="T",[Enum.KeyCode.Y]="Y",[Enum.KeyCode.U]="U",[Enum.KeyCode.I]="I",[Enum.KeyCode.O]="O",[Enum.KeyCode.P]="P",[Enum.KeyCode.A]="A",[Enum.KeyCode.S]="S",[Enum.KeyCode.D]="D",[Enum.KeyCode.F]="F",[Enum.KeyCode.G]="G",[Enum.KeyCode.H]="H",[Enum.KeyCode.J]="J",[Enum.KeyCode.K]="K",[Enum.KeyCode.L]="L",[Enum.KeyCode.Z]="Z",[Enum.KeyCode.X]="X",[Enum.KeyCode.C]="C",[Enum.KeyCode.V]="V",[Enum.KeyCode.B]="B",[Enum.KeyCode.N]="N",[Enum.KeyCode.M]="M",}
...
function update()
    ...
    if page==1 then
        ...
        al.Size=UDim2.new(1,-10,0,40)
        al.Position=UDim2.new(0,5,0,y)
        local isAL=player:GetAttribute("AutoLoad")or false
        al.Text=isAL and"✅ АВТОЗАГРУЗКА ВКЛ"or"⏹ АВТОЗАГРУЗКА ВЫКЛ"
        al.TextColor3=Color3.fromRGB(255,255,255)
        al.BackgroundColor3=isAL and Color3.fromRGB(60,180,60)or Color3.fromRGB(60,60,80)
        ...
        al.MouseButton1Click:Connect(function()
            local cur=player:GetAttribute("AutoLoad")or false
            cur=not cur
            player:SetAttribute("AutoLoad",cur)
            al.Text=cur and"✅ АВТОЗАГРУЗКА ВКЛ"or"⏹ АВТОЗАГРУЗКА ВЫКЛ"
            al.BackgroundColor3=cur and Color3.fromRGB(60,180,60)or Color3.fromRGB(60,60,80)
        end)
        y=y+48
        local inf=Instance.new("TextLabel")
        inf.Size=UDim2.new(1,0,0,50)
        inf.Position=UDim2.new(0,0,0,y)
        inf.Text="Точки сохраняются автоматически"
        inf.TextColor3=Color3.fromRGB(150,150,170)
...
if player:GetAttribute("AutoLoad")then load()end
load()
update()
